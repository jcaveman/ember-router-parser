...
function getRoutesFromRouter(router) {
  try {
    return exports.parseRouter(fs.readFileSync(router));
  } catch (e) {
    console.error(e);
  }
}
...
function parseRouter(router, opts) {
  ...
  var ast = esprima.parse(router, {comment: true, loc: true});
 ...
 * @param {object} prefix - Object telling us the prefixes we will use for the
 *        route names, paths and params from this resource. This is the format:
 *        {
 *          name: 'somePrefix',
 *          path: '/some/prefix',
 *          params: {
...
function addRoute(expressionStatement, prefix) {
  ...
  if (args.length > 1 && args[1].type === OBJECT) {
    ...
    newRoute.path = pathValue;
  } else {
    newRoute.path = '/' + args[0].value;
  }

  var comment = exports.comments.getExpressionComment(expressionStatement);
  if (comment.params) {
    newRoute.doc = comment.main;
    if (prefix && prefix.params) {
      helpers.mergeObjects(prefix.params, comment.params);
      newRoute.params = prefix.params;
    } else {
      newRoute.params = comment.params;
    }
  }
...
function buildPrefix(expression, prefix) {
  ...
  var givenPath = '/' + value;
  var newPrefix;
  var comment = exports.comments.getExpressionComment(expression);
