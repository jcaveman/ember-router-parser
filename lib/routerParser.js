...
function getRouteDataFromObject(obj, originalRoute) {
  var routeName = obj.arguments[0].value; 
  var result = {
    alias: helpers.concatenateAlias(originalRoute.alias, routeName)
  };
  ...
  if (obj.arguments.length > 1) {
    ...
    for (key in obj.arguments) {
      ...
      var path = esprimaHelpers.getPropertyValue(argument, 'path');

      if (path) {
        if (path === '/') {
          return false;
        }
        result.path = helpers.concatenatePath(originalRoute.path, path);
        return result;
      }
    }
  }

  result.path = helpers.concatenatePath(originalRoute.path, routeName);
  return result;
}
